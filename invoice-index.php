...
$invoiceData = $db->getConnection()->prepare('SELECT invoices.*, users.id as user_id, users.surname, invoices.loan, invoices.id as inv_id, laptops.id as laptop_id, laptops.name as laptop_name, invoices.status as inv_stats FROM loans JOIN users ON users.id = loans.customer JOIN invoices ON loans.id = invoices.loan JOIN laptops ON loans.laptop = laptops.id AND loans.customer = ?');
$invoiceData->bind_param('s', $user['id']);
$invoiceData->execute();

if ($result = $invoiceData->get_result()) {
  $invoice = $result->fetch_all(MYSQLI_ASSOC);
  $invoiceData->free_result();
} else {
  $invoice = null;
}
